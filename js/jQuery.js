...
function scroll_page() {
  ...
  $('[data-page="scroll_wrap"]').on("mousewheel DOMMouseScroll", function (e) {
    ...
    if (deltaX > 0 && $(this).prev().attr("data-page") == "scroll_wrap") {
      $("html")
        .stop()
        .animate({
      ...
      deltaX < 0 &&
      $(this).next().attr("data-page") == "scroll_wrap"
    ) {
      $("html")
        .stop()
        .animate({
  ...
  $(window).scroll(function () {
    if ($("header").offset().top == $(window).scrollTop()) {
      ...
      $("section:nth-of-type(1)").offset().top == $(window).scrollTop()
    ) {
      $("nav div").addClass("scroll");
      $("nav div").eq(0).find("span").css("border", "3px solid #8DCEEF");
      $("nav div").eq(1).find("span").css("background-color", "#8DCEEF");
    } else if (
      $("section:nth-of-type(2)").offset().top == $(window).scrollTop()
    ) {
      $("nav div").addClass("scroll");
      $("nav div").eq(0).find("span").css("border", "3px solid white");
      $("nav div").eq(1).find("span").css("background-color", "white");
    } else if (
      $("section:nth-of-type(3)").offset().top == $(window).scrollTop()
    ) {
      $("nav div").addClass("scroll");
      $("nav div").eq(0).find("span").css("border", "3px solid #8DCEEF");
      $("nav div").eq(1).find("span").css("background-color", "#8DCEEF");
    } else if (
      $("section:nth-of-type(4)").offset().top == $(window).scrollTop()
    ) {
      $("nav div").addClass("scroll");
      $("nav div").eq(0).find("span").css("border", "3px solid #8DCEEF");
      $("nav div").eq(1).find("span").css("background-color", "#8DCEEF");
  ...
  $(".wrap > ul li").click(function () {
    ...
    $("html").animate({
      scrollTop: $('[data-page="scroll_wrap"]')
        .eq($(".wrap > ul li.active").index())
        .offset().top,
    });
  });
  ...
  var mstartY = 0,
    mendY = 0;
  $('[data-page="scroll_wrap"]').on("mousedown", function (e) {
    mstartY = e.pageY;
    e.preventDefault();
  });

  $('[data-page="scroll_wrap"]').on("mouseup", function (e) {
    mendY = e.pageY;
    if (
      mstartY - mendY > 50 &&
      $(this).next().attr("data-page") == "scroll_wrap"
      ...
      mendY - mstartY > 50 &&
      $(this).prev().attr("data-page") == "scroll_wrap"
    ) {
      // alert('위');
      $("html")
        .stop()
    ...
    } else if (mstartY - mendY < 50 || mendY - mstartY < 50) {
      // alert('그냥 터치');
    }
  });
}
...
function typingText() {
  ...
  $(window).scroll(function () {
    if (
      $(window).scrollTop() == section01_Top &&
      sct_name.find("div p").text() == 0
    ) {
...
function video_active() {
  ...
  $("#marker").css({
    ...
    top: "-7px",
    "border-radius": "5px",
    width: $("section:nth-of-type(3) ul li b").innerWidth(),
  });
  ...
  ul_list.hover(function (e) {
    ...
    $("#marker").css({
      ...
      top: "-7px",
      "border-radius": "5px",
      "background-color": $("section:nth-of-type(3) ul li.active").attr(
        "data-color"
      ),
    ...
    if (act_location_before < act_location_after) {
      $("section:nth-of-type(3) ol")
        .eq(act_location_before)
        .children("li")
        .css("transform", "translateX(-70px)");
    } else {
      $("section:nth-of-type(3) ol")
        .eq(act_location_before)
        .children("li")
...
function shop_slide() {
  ...
  $("section:nth-of-type(4) div:nth-of-type(2) span")
    .eq(0)
    .on("click", function () {
  ...
  $("section:nth-of-type(4) div:nth-of-type(2) span").on("click", function () {
    let ol_act =
      $("section:nth-of-type(4) ul li").length -
      $("section:nth-of-type(4) ul li.active").length;
